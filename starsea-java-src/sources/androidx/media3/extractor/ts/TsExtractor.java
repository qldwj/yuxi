...
public final class TsExtractor implements Extractor {
    ...
    public int read(ExtractorInput extractorInput, PositionHolder positionHolder) throws IOException {
        ...
        boolean z9 = this.mode == 2;
        if (this.tracksEnded) {
            if (length != -1 && !z9 && !this.durationReader.isDurationReadFinished()) {
        ...
        if (!fillBufferWithAtLeastOnePacket(extractorInput)) {
            for (int i10 = 0; i10 < this.tsPayloadReaders.size(); i10++) {
                ...
                if (tsPayloadReaderValueAt instanceof PesReader) {
                    ...
                    if (pesReader.canConsumeSynthesizedEmptyPusi(z9)) {
        ...
        if (z10) {
            ...
            i12 |= (this.tsPacketBuffer.readUnsignedByte() & 64) != 0 ? 2 : i2;
    ...
    public TsExtractor(int i2, int i10, int i11) {
        this(i2, 1, SubtitleParser.Factory.UNSUPPORTED, new TimestampAdjuster(0L), new DefaultTsPayloadReaderFactory(i10), i11);
    }
    ...
    public TsExtractor(int i2, TimestampAdjuster timestampAdjuster, TsPayloadReader.Factory factory, int i10) {
        this(i2, 1, SubtitleParser.Factory.UNSUPPORTED, timestampAdjuster, factory, i10);
    }

    public TsExtractor(int i2, int i10, SubtitleParser.Factory factory, TimestampAdjuster timestampAdjuster, TsPayloadReader.Factory factory2, int i11) {
        ...
        private TsPayloadReader.EsInfo readEsInfo(ParsableByteArray parsableByteArray, int i2) {
            ...
            while (parsableByteArray.getPosition() < i11) {
                ...
                if (unsignedByte2 == 5) {
                    ...
                    i12 = 135;
                } else if (unsignedByte2 == TS_PMT_DESC_DVB_EXT) {
                    int unsignedByte3 = parsableByteArray.readUnsignedByte();
                    if (unsignedByte3 == 21) {
                        i12 = 172;
                    } else if (unsignedByte3 == 14) {
                        i12 = TsExtractor.TS_STREAM_TYPE_DTS_HD;
                    } else if (unsignedByte3 == 33) {
                        i12 = TsExtractor.TS_STREAM_TYPE_DTS_UHD;
                    }
                } else {
                    if (unsignedByte2 == TS_PMT_DESC_DTS) {
            ...
            return new TsPayloadReader.EsInfo(i12, str, unsignedByte, arrayList, Arrays.copyOfRange(parsableByteArray.getData(), position, i11));
        }
