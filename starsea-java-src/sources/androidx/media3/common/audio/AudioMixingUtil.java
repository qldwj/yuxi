...
public final class AudioMixingUtil {
    ...
    private static float floatSampleToInt16Pcm(float f5) {
        return Util.constrainValue(f5 * (f5 < 0.0f ? DecoderReuseEvaluation.DISCARD_REASON_AUDIO_BYPASS_POSSIBLE : 32767), -32768.0f, 32767.0f);
    }

    private static float getPcmSample(ByteBuffer byteBuffer, boolean z9, boolean z10) {
        if (z10) {
            return z9 ? byteBuffer.getShort() : floatSampleToInt16Pcm(byteBuffer.getFloat());
        }
        return z9 ? int16SampleToFloatPcm(byteBuffer.getShort()) : byteBuffer.getFloat();
    }

    private static float int16SampleToFloatPcm(short s10) {
        return s10 / (s10 < 0 ? DecoderReuseEvaluation.DISCARD_REASON_AUDIO_BYPASS_POSSIBLE : 32767);
    }

    public static ByteBuffer mix(ByteBuffer byteBuffer, AudioProcessor.AudioFormat audioFormat, ByteBuffer byteBuffer2, AudioProcessor.AudioFormat audioFormat2, ChannelMixingMatrix channelMixingMatrix, int i2, boolean z9, boolean z10) {
