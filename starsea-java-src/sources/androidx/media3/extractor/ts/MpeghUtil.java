...
final class MpeghUtil {
    ...
    public static class Mpegh3daConfig {
        ...
        private Mpegh3daConfig(int i2, int i10, int i11, byte[] bArr) {
    ...
    private static int readEscapedIntValue(ParsableBitArray parsableBitArray, int i2, int i10, int i11) {
        Assertions.checkArgument(Math.max(Math.max(i2, i10), i11) <= 31);
        int i12 = (1 << i2) - 1;
        int i13 = (1 << i10) - 1;
        ...
        if (bits == i12) {
            ...
            if (bits2 == i13) {
                ...
                return parsableBitArray.readBits(i11) + bits;
            }
        }
        return bits;
    }

    private static long readEscapedLongValue(ParsableBitArray parsableBitArray, int i2, int i10, int i11) {
        Assertions.checkArgument(Math.max(Math.max(i2, i10), i11) <= 63);
        long j10 = (1 << i2) - 1;
        long j11 = (1 << i10) - 1;
        l.A(l.A(j10, j11), 1 << i11);
        if (parsableBitArray.bitsLeft() < i2) {
            return -1L;
        }
        ...
        if (bitsToLong == j10) {
            ...
            if (bitsToLong2 == j11) {
                ...
                return parsableBitArray.readBitsToLong(i11) + bitsToLong;
            }
        }
        return bitsToLong;
    }

    private static void skipMpegh3daDecoderConfig(ParsableBitArray parsableBitArray, int i2, int i10) {
        int bits;
