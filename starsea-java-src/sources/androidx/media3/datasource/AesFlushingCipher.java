package androidx.media3.datasource;

import androidx.media3.common.util.Assertions;
import androidx.media3.common.util.UnstableApi;
import androidx.media3.common.util.Util;
import java.nio.ByteBuffer;
import java.security.InvalidAlgorithmParameterException;
import java.security.InvalidKeyException;
import java.security.NoSuchAlgorithmException;
import javax.crypto.Cipher;
import javax.crypto.NoSuchPaddingException;
import javax.crypto.ShortBufferException;
import javax.crypto.spec.IvParameterSpec;
import javax.crypto.spec.SecretKeySpec;

/* JADX INFO: compiled from: r8-map-id-b2022710361550930af238f1d63db3afb833aa2031d23bf31bcf974bb7c3ceaa */
/* JADX INFO: loaded from: /workspace/starsea/classes.dex */
@UnstableApi
public final class AesFlushingCipher {
    private final int blockSize;
    private final Cipher cipher;
    private final byte[] flushedBlock;
    private int pendingXorBytes;
    private final byte[] zerosBlock;

    public AesFlushingCipher(int i2, byte[] bArr, String str, long j10) {
        this(i2, bArr, getFNV64Hash(str), j10);
    }

    private static long getFNV64Hash(String str) {
        long j10 = 0;
        if (str == null) {
            return 0L;
        }
        for (int i2 = 0; i2 < str.length(); i2++) {
            long jCharAt = j10 ^ ((long) str.charAt(i2));
            j10 = jCharAt + (jCharAt << 1) + (jCharAt << 4) + (jCharAt << 5) + (jCharAt << 7) + (jCharAt << 8) + (jCharAt << 40);
        }
        return j10;
    }

    private byte[] getInitializationVector(long j10, long j11) {
        return ByteBuffer.allocate(16).putLong(j10).putLong(j11).array();
    }

    private int nonFlushingUpdate(byte[] bArr, int i2, int i10, byte[] bArr2, int i11) {
        try {
            return this.cipher.update(bArr, i2, i10, bArr2, i11);
        } catch (ShortBufferException e10) {
            throw new RuntimeException(e10);
        }
    }

    public void update(byte[] bArr, int i2, int i10, byte[] bArr2, int i11) {
        int i12 = i2;
        int i13 = i10;
        int i14 = i11;
        do {
            int i15 = this.pendingXorBytes;
            if (i15 <= 0) {
                int iNonFlushingUpdate = nonFlushingUpdate(bArr, i12, i13, bArr2, i14);
                if (i13 == iNonFlushingUpdate) {
                    return;
                }
                int i16 = i13 - iNonFlushingUpdate;
                int i17 = 0;
                Assertions.checkState(i16 < this.blockSize);
                int i18 = i14 + iNonFlushingUpdate;
                int i19 = this.blockSize - i16;
                this.pendingXorBytes = i19;
                Assertions.checkState(nonFlushingUpdate(this.zerosBlock, 0, i19, this.flushedBlock, 0) == this.blockSize);
                while (i17 < i16) {
                    bArr2[i18] = this.flushedBlock[i17];
                    i17++;
                    i18++;
                }
                return;
            }
            bArr2[i14] = (byte) (bArr[i12] ^ this.flushedBlock[this.blockSize - i15]);
            i14++;
            i12++;
            this.pendingXorBytes = i15 - 1;
            i13--;
        } while (i13 != 0);
    }

    public void updateInPlace(byte[] bArr, int i2, int i10) {
        update(bArr, i2, i10, bArr, i2);
    }

    public AesFlushingCipher(int i2, byte[] bArr, long j10, long j11) {
        try {
            Cipher cipher = Cipher.getInstance("AES/CTR/NoPadding");
            this.cipher = cipher;
            int blockSize = cipher.getBlockSize();
            this.blockSize = blockSize;
            this.zerosBlock = new byte[blockSize];
            this.flushedBlock = new byte[blockSize];
            long j12 = j11 / ((long) blockSize);
            int i10 = (int) (j11 % ((long) blockSize));
            cipher.init(i2, new SecretKeySpec(bArr, Util.splitAtFirst(cipher.getAlgorithm(), "/")[0]), new IvParameterSpec(getInitializationVector(j10, j12)));
            if (i10 != 0) {
                updateInPlace(new byte[i10], 0, i10);
            }
        } catch (InvalidAlgorithmParameterException | InvalidKeyException | NoSuchAlgorithmException | NoSuchPaddingException e10) {
            throw new RuntimeException(e10);
        }
    }
}
