...
public final class DtsUtil {
    ...
    public static final class DtsHeader {
        ...
        private DtsHeader(String str, int i2, int i10, int i11, long j10, int i12) {
            this.mimeType = str;
    ...
    private static void checkCrc(byte[] bArr, int i2) throws ParserException {
        int i10 = i2 - 2;
        if (((bArr[i2 - 1] & FIRST_BYTE_14B_LE) | ((bArr[i10] << 8) & 65535)) != Util.crc16(bArr, 0, i10, 65535)) {
    ...
    public static int getDtsFrameSize(byte[] bArr) {
        ...
        if (b12 != -2) {
            if (b12 == -1) {
                ...
                i2 = ((bArr[5] & 3) << 12) | ((bArr[6] & FIRST_BYTE_14B_LE) << 4);
                b10 = bArr[7];
            } else {
        ...
        i2 = ((bArr[4] & 3) << 12) | ((bArr[7] & FIRST_BYTE_14B_LE) << 4);
        b10 = bArr[6];
        i10 = (((b10 & 240) >> 4) | i2) + 1;
        if (z9) {
            return (i10 * 16) / 14;
    ...
    private static ParsableBitArray getNormalizedFrame(byte[] bArr) {
        ...
        if (isLittleEndianFrameHeader(bArrCopyOf)) {
            for (int i2 = 0; i2 < bArrCopyOf.length - 1; i2 += 2) {
                byte b11 = bArrCopyOf[i2];
                int i10 = i2 + 1;
                bArrCopyOf[i2] = bArrCopyOf[i10];
                bArrCopyOf[i10] = b11;
            }
        }
    ...
    public static Format parseDtsFormat(byte[] bArr, String str, String str2, int i2, DrmInitData drmInitData) {
        ...
        return new Format.Builder().setId(str).setSampleMimeType(MimeTypes.AUDIO_DTS).setAverageBitrate(i12).setChannelCount(i10 + (normalizedFrame.readBits(2) > 0 ? 1 : 0)).setSampleRate(i11).setDrmInitData(drmInitData).setLanguage(str2).setRoleFlags(i2).build();
    }
    ...
    public static DtsHeader parseDtsHdHeader(byte[] bArr) throws ParserException {
        ...
        if (bit) {
            ...
            i11 = SAMPLE_RATE_BY_INDEX[normalizedFrame.readBits(4)];
        ...
        return new DtsHeader(MimeTypes.AUDIO_DTS_EXPRESS, bits4, i16, bits3, jScaleLargeTimestamp, 0);
    }
    ...
    public static DtsHeader parseDtsUhdHeader(byte[] bArr, AtomicInteger atomicInteger) throws ParserException {
        ...
        if (i11 == 0) {
            ...
            int bits3 = (normalizedFrame.readBits(3) + 1) * i2;
        ...
        return new DtsHeader(MimeTypes.AUDIO_DTS_X, 2, i12, unsignedVarInt2 + (atomicInteger2.get() != 0 ? parseUnsignedVarInt(normalizedFrame, UHD_AUDIO_CHUNK_SIZE_LENGTH_TABLE, true) : 0) + unsignedVarInt, j10, 0);
    }
    ...
    private static int parseUnsignedVarInt(ParsableBitArray parsableBitArray, int[] iArr, boolean z9) {
        int i2 = 0;
        int i10 = 0;
        ...
        return parsableBitArray.readBits(iArr[i10]) + i2;
    }
