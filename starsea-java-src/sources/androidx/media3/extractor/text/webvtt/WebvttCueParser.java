...
public final class WebvttCueParser {
    ...
    public static final class WebvttCueInfoBuilder {
        ...
        private static float computeLine(float f5, int i2) {
            if (f5 != -3.4028235E38f && i2 == 0 && (f5 < 0.0f || f5 > 1.0f)) {
                return 1.0f;
            }
            if (f5 != -3.4028235E38f) {
                return f5;
        ...
        private static Layout.Alignment convertTextAlignment(int i2) {
            if (i2 != 1) {
                ...
                if (i2 != 3) {
                    if (i2 != 4) {
                        if (i2 != 5) {
                            b.M("Unknown textAlignment: ", WebvttCueParser.TAG, i2);
                            return null;
                        }
                    }
                }
        ...
        private static float deriveMaxSize(int i2, float f5) {
            if (i2 == 0) {
                return 1.0f - f5;
            }
            if (i2 == 1) {
    ...
    private static void applyDefaultColors(SpannableStringBuilder spannableStringBuilder, Set<String> set, int i2, int i10) {
        for (String str : set) {
            Map<String, Integer> map = DEFAULT_TEXT_COLORS;
            if (map.containsKey(str)) {
                spannableStringBuilder.setSpan(new ForegroundColorSpan(map.get(str).intValue()), i2, i10, 33);
            } else {
                Map<String, Integer> map2 = DEFAULT_BACKGROUND_COLORS;
                if (map2.containsKey(str)) {
                    spannableStringBuilder.setSpan(new BackgroundColorSpan(map2.get(str).intValue()), i2, i10, 33);
                }
            }
        }
    }
    ...
    private static void applyRubySpans(SpannableStringBuilder spannableStringBuilder, String str, StartTag startTag, List<Element> list, List<WebvttCssStyle> list2) {
        ...
        for (int i10 = 0; i10 < arrayList.size(); i10++) {
            if (TAG_RUBY_TEXT.equals(((Element) arrayList.get(i10)).startTag.name)) {
                ...
                spannableStringBuilder.setSpan(new RubySpan(charSequenceSubSequence.toString(), iFirstKnownRubyPosition), i2, i11, 33);
    ...
    private static void applySpansForTag(String str, StartTag startTag, List<Element> list, SpannableStringBuilder spannableStringBuilder, List<WebvttCssStyle> list2) {
        ...
        for (int i10 = 0; i10 < applicableStyles.size(); i10++) {
            applyStyleToText(spannableStringBuilder, applicableStyles.get(i10).style, i2, length);
        }
    }

    private static void applyStyleToText(SpannableStringBuilder spannableStringBuilder, WebvttCssStyle webvttCssStyle, int i2, int i10) {
        if (webvttCssStyle == null) {
            return;
        }
        if (webvttCssStyle.getStyle() != -1) {
            SpanUtil.addOrReplaceSpan(spannableStringBuilder, new StyleSpan(webvttCssStyle.getStyle()), i2, i10, 33);
        }
        if (webvttCssStyle.isLinethrough()) {
            spannableStringBuilder.setSpan(new StrikethroughSpan(), i2, i10, 33);
        }
        if (webvttCssStyle.isUnderline()) {
            spannableStringBuilder.setSpan(new UnderlineSpan(), i2, i10, 33);
        }
        if (webvttCssStyle.hasFontColor()) {
            SpanUtil.addOrReplaceSpan(spannableStringBuilder, new ForegroundColorSpan(webvttCssStyle.getFontColor()), i2, i10, 33);
        }
        if (webvttCssStyle.hasBackgroundColor()) {
            SpanUtil.addOrReplaceSpan(spannableStringBuilder, new BackgroundColorSpan(webvttCssStyle.getBackgroundColor()), i2, i10, 33);
        }
        if (webvttCssStyle.getFontFamily() != null) {
            SpanUtil.addOrReplaceSpan(spannableStringBuilder, new TypefaceSpan(webvttCssStyle.getFontFamily()), i2, i10, 33);
        }
        int fontSizeUnit = webvttCssStyle.getFontSizeUnit();
        if (fontSizeUnit == 1) {
            SpanUtil.addOrReplaceSpan(spannableStringBuilder, new AbsoluteSizeSpan((int) webvttCssStyle.getFontSize(), true), i2, i10, 33);
        } else if (fontSizeUnit == 2) {
            SpanUtil.addOrReplaceSpan(spannableStringBuilder, new RelativeSizeSpan(webvttCssStyle.getFontSize()), i2, i10, 33);
        } else if (fontSizeUnit == 3) {
            SpanUtil.addOrReplaceSpan(spannableStringBuilder, new RelativeSizeSpan(webvttCssStyle.getFontSize() / 100.0f), i2, i10, 33);
        }
        if (webvttCssStyle.getCombineUpright()) {
            spannableStringBuilder.setSpan(new HorizontalTextInVerticalContextSpan(), i2, i10, 33);
        }
    }
    ...
    private static int firstKnownRubyPosition(int i2, int i10, int i11) {
        if (i2 != -1) {
            return i2;
        }
        if (i10 != -1) {
        ...
        while (i2 < str2.length()) {
            ...
            if (cCharAt == '&') {
                ...
                if (iFindEndOfTag < str2.length()) {
                    boolean z9 = str2.charAt(iFindEndOfTag) == '/';
