...
final class TtmlNode {
    ...
    private void applyStyleToOutput(Map<String, TtmlStyle> map, Cue.Builder builder, int i2, int i10, int i11) {
        ...
        if (ttmlStyleResolveStyle != null) {
            TtmlRenderUtil.applyStylesToSpan(spannableStringBuilder2, i2, i10, ttmlStyleResolveStyle, this.parent, map, i11);
    ...
    private void getEventTimes(TreeSet<Long> treeSet, boolean z9) {
        boolean zEquals = TAG_P.equals(this.tag);
        boolean zEquals2 = TAG_DIV.equals(this.tag);
        if (z9 || zEquals || (zEquals2 && this.imageId != null)) {
        ...
        for (int i2 = 0; i2 < this.children.size(); i2++) {
            this.children.get(i2).getEventTimes(treeSet, z9 || zEquals);
        }
    }
    ...
    private void traverseForImage(long j10, String str, List<Pair<String, String>> list) {
        ...
        for (int i2 = 0; i2 < getChildCount(); i2++) {
            getChild(i2).traverseForImage(j10, str, list);
        }
    }
    ...
    private void traverseForStyle(long j10, Map<String, TtmlStyle> map, Map<String, TtmlRegion> map2, String str, Map<String, Cue.Builder> map3) {
        if (isActive(j10)) {
            ...
            for (int i2 = 0; i2 < getChildCount(); i2++) {
                getChild(i2).traverseForStyle(j10, map, map2, str2, map3);
            }
        }
    }

    private void traverseForText(long j10, boolean z9, String str, Map<String, Cue.Builder> map) {
        ...
        if (isActive(j10)) {
            ...
            while (i2 < getChildCount()) {
                getChild(i2).traverseForText(j10, z9 || zEquals, str2, map);
                i2++;
                j10 = j10;
                map = map;
            }
            Map<String, Cue.Builder> map2 = map;
            if (zEquals) {
                TtmlRenderUtil.endParagraph(getRegionOutputText(str2, map2));
            }
            for (Map.Entry<String, Cue.Builder> entry2 : map2.entrySet()) {
